...
def simulate(thrust, init_pos, planet_poses, end_time ):
    ...
    def get_force_on_ship(pos):
        ...
        for i in range(len(planet_poses)):
            force += -planet_masses[i]*(pos - planet_poses[i])/(np.linalg.norm(pos-end_pos)+0.0001)**3
        return force
    while t < end_time:
        #print(pos, t)
...
for i in range(10):
    ...
    for thrust in thrusts:
        trajectory, how_close, at_what_time, at_what_thrust = simulate(thrust, init_pos, planet_positions, 10)
